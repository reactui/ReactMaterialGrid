...
export default class DataManager {
  ...
  treeDataMaxLevel = 0;
  groupedDataLength = 0;
  defaultExpanded: Function | boolean = false;
  bulkEditOpen = false;
  bulkEditChangedRows: any = [];
  ...
  setColumns(columns: any) {
    const undefinedWidthColumns = columns.filter(
      (c: any) => c.width === undefined && !c.hidden
    );
    let usedWidth = ["0px"];
    ...
    let calculatedUsedWidth: string = "(" + usedWidth.join(" + ") + ")";
    undefinedWidthColumns.forEach((columnDef: any) => {
      columnDef.tableData.width =
        columnDef.tableData.initialWidth = `calc((100% - ${calculatedUsedWidth}) / ${undefinedWidthColumns.length})`;
    });
  }
  ...
  changeParentFunc(parentFunc: Function) {
  ...
  changeDetailPanelVisibility(path: string, render: any) {
    const rowData = this.findDataByPath(this.sortedData, path);
  ...
  changeColumnHidden(column: any, hidden: boolean) {
  ...
  startCellEditable = (rowData: any, columnDef: any) => {
  ...
  finishCellEditable = (rowData: any, columnDef: any) => {
  ...
  onBulkEditRowChanged = (oldData: any, newData: any) => {
  ...
  findDataByPath = (renderData: any, path: any) => {
    if (this.isDataType("tree")) {
      ...
      const node = path.reduce((result: any, current: number) => {
  ...
  findGroupByGroupPath(renderData: any, path: any) {
  ...
  sortList(list: any) {
    ...
    if (columnDef.customSort) {
      if (this.orderDirection === "desc") {
        result = list.sort((a: any, b: any) =>
          columnDef.customSort(b, a, "row", "desc")
        );
      } else {
        result = list.sort((a: any, b: any) =>
  ...
  searchData = () => {
    ...
    if (this.searchText && this.applySearch) {
      ...
      this.searchedData = this.searchedData.filter((row: any) => {
        return this.columns
          .filter((columnDef) => {
  ...
  treefyData() {
    this.sorted = this.paged = false;
    this.data.forEach((a: any) => (a.tableData.childRows = null));
    this.treefiedData = [];
    ...
    const markForTreeRemove = (rowData: any) => {
      let pointer = this.treefiedData;
      rowData.tableData.path.forEach((pathPart: any) => {
    ...
    const traverseChildrenAndUnmark = (rowData: any) => {
      if (rowData.tableData.childRows) {
        rowData.tableData.childRows.forEach((row: any) => {
  ...
  sortData() {
    ...
    if (this.isDataType("group")) {
      ...
      const sortGroups = (list: [], columnDef: any) => {
        if (columnDef.customSort) {
          return list.sort(
            columnDef.tableData.groupSort === "desc"
              ? (a: any, b: any) =>
                  columnDef.customSort(b.value, a.value, "group")
              : (a: any, b: any) =>
                  columnDef.customSort(a.value, b.value, "group")
          );
        } else {
          return list.sort(
            columnDef.tableData.groupSort === "desc"
      ...
      const sortGroupData = (list: [], level: number) => {
